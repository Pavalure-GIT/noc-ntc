resource "aws_autoscaling_group" "app" {
  ...
  tags = ["${
      ...
      list(
        map(
          "key", "Name",
          "value", "NTC Kong API GW Autoscaling group - ${terraform.workspace}",
...
EOF
  ...
  vars {
    pg_user       = "${aws_db_instance.default_nolntc_ntc.username}"
    pg_password   = "${aws_db_instance.default_nolntc_ntc.password}"
    pg_database   = "${aws_db_instance.default_nolntc_ntc.name}"
    pg_endpoint   = "${aws_db_instance.default_nolntc_ntc.address}"
    secret        = "${data.aws_ssm_parameter.kongpassword.value}"
    gateway       = "http://${aws_alb.apigw_ntc.dns_name}:8001/apis"
    name          = "kongNNTC"
    upstream_url  = "https://${data.terraform_remote_state.ntc.ntc_alb_dns}"
...
resource "aws_launch_configuration" "ecs_launch_configuration" {
  name_prefix          = "${var.name}-${terraform.workspace}-ecs"
